...
class SignIn: UIViewController, ValidationDelegate, UITextFieldDelegate  {
    ...
    func validationSuccessful() {
        ...
        AuthServices.login(email: self.emailTextField.text!, password: self.passwordTextField.text!) { (user, error) in
            
            self.view.hideHud()
            
            if error == nil {
                UserServices.getAllUSers(comletion: { (error) in })
                self.performSegue(withIdentifier: "goToAllUsers", sender: nil)
            }
        }
